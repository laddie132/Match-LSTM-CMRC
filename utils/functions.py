...
def beam_search(answer_prop, k):
    """
    beam search on predict step
    :param answer_prop: (batch, ans_len, context_len)
    :return: (batch, k, ans_len)
    """

    batch, ans_len, seq_len = answer_prop.shape

    def _gen_mask(cur_idx):
        cur_mask = []
        for i in range(cur_idx.shape[0]):
            start_len = cur_idx[i].item()
            m = torch.cat([answer_prop.new_zeros(start_len),
                           answer_prop.new_ones(seq_len - start_len)])
            cur_mask.append(m)
        cur_mask = torch.stack(cur_mask, dim=0)
        return cur_mask

    assert k <= seq_len

    s_prop = answer_prop[:, 0, :]
    e_prop = answer_prop[:, 1, :]

    s_sort, s_idx = torch.sort(s_prop, dim=1, descending=True)
    ...
    for i in range(k):
        ...
        cur_s_mask = _gen_mask(cur_s_idx)
        cur_s_pad = cur_s.unsqueeze(-1) * cur_s_mask

        cur_s_e = cur_s_pad * e_prop
        cur_s_e_sort, cur_e_idx = torch.sort(cur_s_e, dim=1, descending=True)
        cand_ans_prop.append(cur_s_e_sort[:, :k])

        cur_ans = [torch.stack([cur_s_idx, cur_e_idx[:, j]], dim=-1) for j in range(k)]
        cand_ans += cur_ans
